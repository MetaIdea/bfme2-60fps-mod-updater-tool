...
function LoadMacros(file)
	...
	local i = 1
	for i=1,#TextFileTable,1 do
		local number = ExtractSingleNumber(TextFileTable[i])
		if string.find(TextFileTable[i],"#define") and number then
			table.insert(MACROS, {string.match(TextFileTable[i], "%s+(%S+)"), number})
			--print(MACROS[#MACROS][1] .. " " .. MACROS[#MACROS][2])
		end
	end
end
...
function ReplaceMacros(file)
	print(file)
	local TextFileTable = ParseTextFileIntoTable(file) 
	for i=1,#TextFileTable,1 do
		for j=1,#MACROS,1 do
			if isWordFoundInString(MACROS[j][1],TextFileTable[i]) then
			--if string.find(TextFileTable[i], MACROS[j][1]) and  (string.find(TextFileTable[i], MACROS[j][1] .. " ") or string.find(TextFileTable[i], MACROS[j][1] .. [[\r\n]]) or string.find(TextFileTable[i], MACROS[j][1] .. "\n") or string.find(TextFileTable[i], MACROS[j][1] .. "\r\t")) then
				print(MACROS[j][1])
				TextFileTable[i] = string.gsub(TextFileTable[i], MACROS[j][1], MACROS[j][2])
			end
		end
	end
	WriteToFile(ConvertLineTableToText(TextFileTable), file)	
...
function MultMinMaxVal(file)
	print(file)
	local TextFileTable = ParseTextFileIntoTable(file) 
	for i=1,#TextFileTable,1 do
		if string.find(TextFileTable[i], "Min:") or string.find(TextFileTable[i], "Max:") then
			TextFileTable[i] = string.gsub(TextFileTable[i], "Min:", "Min: ")
			TextFileTable[i] = string.gsub(TextFileTable[i], "Max:", "Max: ")
			local numtable = ExtractNumbersFromLine(TextFileTable[i])
			for j=1,#numtable,1  do
				TextFileTable[i] = string.gsub(TextFileTable[i], numtable[j], 2*tonumber(numtable[j]))
			end
			TextFileTable[i] = string.gsub(TextFileTable[i], "Min: ", "Min:")
			TextFileTable[i] = string.gsub(TextFileTable[i], "Max: ", "Max:")
		end
...
MultMinMaxVal("ValueScale60FPS.ini")

-- LoadMacros("MACROS.ini")
-- ReplaceMacros("ValueScale60FPS.ini")
-- DoMathOperationsOnMultipleNumbersInLines("ValueScale60FPS.ini", StandardValueScaleContainer)
